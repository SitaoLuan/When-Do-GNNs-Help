...
from gntk_test import gntk_homophily_, gntk_homophily,dual_activation_dev,dual_activation
from utils_large import random_disassortative_splits, accuracy
...
def node_homophily(A, labels):
    ...
    """
    src_node, targ_node = A.coalesce().indices()[0,:], A.coalesce().indices()[1,:] #A.to_dense().nonzero()[:,0], A.to_dense().nonzero()[:,1]##A.nonzero()
...
def class_distribution(A, labels):
    edge_index = A.coalesce().indices()#remove_self_loops(A.coalesce().indices())[0]
    src_node, targ_node = edge_index[0,:], edge_index[1,:]
    ...
    for i in range(labels.max()+1):
        p_bar[i] = torch.sum(deg[torch.where(labels == i)])#/src_node.shape[0]
        
        for j in range(labels.max()+1):
            # print(torch.sum(labels[targ_node[torch.where(labels[src_node]==i)]] ==  j),  src_node.shape[0])
            pc[i,j]  = torch.sum(labels[targ_node[torch.where(labels[src_node]==i)]] ==  j)#/src_node.shape[0]
            # print(pc[i,j])
...
def generalized_edge_homophily(adj, features, label, sample_max = 75000, iteration = 10):
    ...
    if nedges < sample_max:
        ...
        # print("torch.sum(adj): ", torch.sum(adj))
        ...
        for i in  range(iteration):
            # sample, _, _ = random_disassortative_splits(label, label.max()+1, sample_max/nnodes)
            sample = torch.tensor(random.sample(list(np.arange(adj.coalesce().indices()[0,:].shape[0])), int(sample_max)))
            src_node, targ_node = adj.coalesce().indices()[0,:][sample], adj.coalesce().indices()[1,:][sample]
            # sim = torch.tensor(cosine_similarity(features[sample].cpu(),features[sample].cpu()))#.to(device)
            sim  = torch.sum(features[src_node].cpu()*features[targ_node].cpu(),1)/(torch.norm(features[src_node].cpu(),dim=1,p=2)*torch.norm(features[targ_node].cpu(),dim=1,p=2))
            sim[torch.isnan(sim)] = 0
            # print(adj.shape,sample.shape)
            # adj_sample = torch.spmm(adj, torch.eye(nnodes))[sample,:][:,sample] #.to_dense()s
            # adj_sample = adj_sample - torch.diag(torch.diag(adj_sample))
            # adj_sample = (adj_sample>0).float()
            g_homo[i] = torch.mean(sim)#torch.sum(sim * adj_sample)/torch.sum(adj_sample)
            # print(edge_homo[i])
...
def similarity(features, adj, label, NTK = None, hard = None, LP = 1, ifsum = 1, idx_train = None):
    if str(type(idx_train)) == '<class \'NoneType\'>':
        ...
        inner_prod = inner_prod = torch.mm(torch.spmm(adj, features)[idx_train,:], torch.spmm(adj, features)[idx_train,:].transpose(0,1))
    for i in range(labels.max()+1):
        # Think about using torch.sum or torch.mean
        if ifsum == 1:
            # print(inner_prod.shape, labels.shape)
    ...
    if hard == None:
        ...
        if LP == 1:
            # weight mean
            LAF_ratio = (weight_matrix[np.arange(labels.size(0)),labels]/degs_label)/((torch.sum(weight_matrix,1)-weight_matrix[np.arange(labels.size(0)),labels])/(nnodes-degs_label))
            LAF_ratio[torch.isnan(LAF_ratio)]=0
            return torch.mean((LAF_ratio>=1).float())#
        else:
            return torch.mean(((torch.sum(weight_matrix - weight_matrix*label,1)<=0) & (torch.sum(weight_matrix*label,1)>=0)).float())
    else:
        if LP == 1:
            return torch.mean(torch.argmax(weight_matrix,1).eq(labels).float())
        else:
            return torch.mean(((torch.max(weight_matrix - weight_matrix*label,1)[0] <=0.) & (torch.sum(weight_matrix*label,1)>=0)).float())
...
def gntk_homophily_(features, adj, label, sample, n_layers, kernel_regression = False):
    ...
    G_gram = torch.mm(torch.spmm(adj, features)[sample,:], torch.transpose(torch.spmm(adj, features)[sample,:],0,1))#sample_agg_features#torch.mm(sample_agg_features, torch.transpose(sample_agg_features,0,1))
    ...
    if n_layers == 1:
        ...
        K_G = 1/pi*(G_gram*(pi-arccos)+ sqrt) # + gram/pi*(pi-arccos)
    else:
        K_G = G_gram 
...
def classifier_based_performance_metric(features, adj, labels, sample_max, base_classifier = 'kernel_reg1', epochs = 100):
    ...
    # label_onehot_full = torch.eye(labels.max()+1)[labels]
    G_results, X_results, diff_results, G_good_p_results, X_good_p_results = torch.zeros(epochs), torch.zeros(epochs), torch.zeros(epochs), torch.zeros(epochs), torch.zeros(epochs)
    t_time = time.time()
    for j in range(epochs):
        ...
        if base_classifier in {'kernel_reg0', 'kernel_reg1'}:
            ...
            Kreg_G, Kreg_X = K_graph_val_train.cpu()@(torch.tensor(np.linalg.pinv(K_graph_train_train.cpu().numpy()))@label_onehot.cpu()[idx_train]), K_val_train.cpu()@(torch.tensor(np.linalg.pinv(K_train_train.cpu().numpy()))@label_onehot.cpu()[idx_train])
            diff_results[j] = (accuracy(labels_sample[idx_val], Kreg_G) > accuracy(labels_sample[idx_val],  Kreg_X))
            G_results[j] = accuracy(labels_sample[idx_val], Kreg_G) ##accuracy(labels_sample[idx_val], K_graph_val_train.cpu()@(torch.tensor(np.linalg.pinv(K_graph_train_train.cpu().numpy()))@label_onehot.cpu()[idx_train])) 
            X_results[j] = accuracy(labels_sample[idx_val],  Kreg_X) #accuracy(labels_sample[idx_val], K_val_train.cpu()@(torch.tensor(np.linalg.pinv(K_train_train.cpu().numpy()))@label_onehot.cpu()[idx_train]) )
            ...
            diff_results[j] = (torch.mean(G_pred.eq(labels_sample[idx_val]).float()) > torch.mean(X_pred.eq(labels_sample[idx_val]).float()))
            G_results[j] = torch.mean(G_pred.eq(labels_sample[idx_val]).float())##accuracy(labels_sample[idx_val], K_graph_val_train.cpu()@(torch.tensor(np.linalg.pinv(K_graph_train_train.cpu().numpy()))@label_onehot.cpu()[idx_train])) 
            X_results[j] = torch.mean(X_pred.eq(labels_sample[idx_val]).float()) #accuracy(labels_sample[idx_val], K_val_train.cpu()@(torch.tensor(np.linalg.pinv(K_train_train.cpu().numpy()))@label_onehot.cpu()[idx_train]) )
        else:
            #  SVM based p-values
            X = features[sample].cpu()
            X_agg = torch.spmm(adj, features)[sample].cpu()
    ...
    if scipy.__version__ == '1.4.1':
        # print(X_results,G_results)
        g_aware_good_stats, g_aware_good_p = ttest_ind(X_results.detach().cpu(), G_results.detach().cpu(), axis=0, equal_var=False, nan_policy='propagate')#ttest_1samp(diff_results,0.5)
    else:
        g_aware_good_stats, g_aware_good_p = ttest_ind(X_results.detach().cpu(), G_results.detach().cpu(), axis=0, equal_var=False, nan_policy='propagate')
        
    
    # class_size, class_prop = [],[]
    
    # for j in np.arange(labels.max().cpu()+1):
    #     class_size.append(torch.sum(labels==j))
    #     class_prop.append(torch.sum(labels==j)/nnodes)
    ...
    # print(base_dataset, base_classifier,'Sample Max: ', sample_max, "G aware models good stats: ", g_aware_good_stats, 
    #       "G aware models good p values: ", g_aware_good_p, "mean: ", torch.mean(diff_results), "std: ", torch.std(diff_results),
    #       'total time: ', time.time()-t_time)
