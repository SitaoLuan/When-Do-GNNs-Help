...
large_datasets = ['deezer-europe', 'yelp-chi', 'Penn94', 'arxiv-year', 'pokec', 'snap-patents', "genius",
                  "twitch-gamer", "wiki"]
...
def train(model, optimizer, adj_low, adj_high, adj_low_unnormalized, adj_label, features, labels, idx_train, idx_val,
          criterion, dataset_name):
    model.train()
    optimizer.zero_grad()
...
def load_geometric_crocodile():
    ...
    print("Load geometric crocodile | %d nodes | %d edges | %d features | continuous y" % (
        adj.shape[0], adj.sum(), len(x[0])))
    features = x
...
def Prop(output, labels, idx_train, idx_val, idx_test, sample_max=5000):
    ...
    labels_val_train = torch.cat([labels_val, labels_train])
    ...
    for i in range(labels_val.shape[0]):
        ...
        stat, p = ttest_ind(a.detach(), b.detach(), axis=0, equal_var=False,
