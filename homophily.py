...
def classifier_based_performance_metric(features, adj, labels, sample_max, rcond=1e-15, base_classifier = 'kernel_reg1', epochs = 100):
    nnodes = (labels.shape[0])
    if labels.dim() >1:
    ...
    for j in range(epochs):
        ...
        if base_classifier in {'kernel_reg0', 'kernel_reg1'}:
            ...
            Kreg_G, Kreg_X = K_graph_val_train.cpu()@(torch.tensor(np.linalg.pinv(K_graph_train_train.cpu().numpy(),rcond=rcond))@label_onehot.cpu()[idx_train]), K_val_train.cpu()@(torch.tensor(np.linalg.pinv(K_train_train.cpu().numpy(),rcond=rcond))@label_onehot.cpu()[idx_train])
            
            
            diff_results[j] = (torch.mean(Kreg_G.argmax(1).eq(labels_sample[idx_val]).float()) > torch.mean(Kreg_X.argmax(1).eq(labels_sample[idx_val]).float()) )
